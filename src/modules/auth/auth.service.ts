...
import { sendResetEmail } from "../../common/utils/mailer.js";

export async function registerUser(name: string, email: string, password: string) {
  const existingUser = await findUserByEmail(email);
  if (existingUser) {
    internalServerException({ message: "email already exists", statusCode: 400 });
  }
...
export async function loginUser(email: string, password: string) {
  ...
  if (!user) {
    internalServerException({ message: "invalid credentials", statusCode: 401 });
    return;
  }
...
export async function resetPassword(email: string, code: string, newPassword: string) {
  ...
  if (!user) {
    internalServerException({ message: "invalid or expired code", statusCode: 400 });
    return;
  }
